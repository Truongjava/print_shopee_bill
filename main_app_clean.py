...
def run_automation(cookie_path, output_dir, max_orders, log_cb, state_cb, stop_event=None,
    ...
    try:
        ...
        # ── 3. Kiểm tra có đơn không ──
        # Dùng carrier_counts nếu có, nếu không thì đếm checkbox
        if carrier_counts and carrier in carrier_counts:
            total_avail = carrier_counts[carrier]
        else:
            total_avail = page.evaluate(
        ...
        log_cb(f'  ✓ Đã chọn {checked}/{total_avail} đơn hàng', 'ok')
        ...
        if test_mode:
            ...
            try:
                ss = str(Path(output_dir) / f'test_mode_{carrier or "all"}.png')
                page.screenshot(path=ss)
                log_cb(f'  📸 Screenshot: {ss}', 'info')
            except Exception:
                pass
        ...
        if tao_btn:
            ...
            log_cb('  ⚠ Không tìm thấy nút "Tạo" — thử tiếp...', 'warn')

        # ── 7. Chọn loại phiếu: "Phiếu xuất hàng + Phiếu gửi hàng và phiếu đóng gói" ──
        state_cb('printing', 'Đang chọn loại phiếu...')
        selected_doc = False
        for doc_label in [
            'Phiếu xuất hàng + Phiếu gửi hàng và phiếu đóng gói',
            'Phiếu xuất hàng',
            'Phiếu gửi hàng và phiếu đóng gói',
        ]:
            try:
                lbl = page.locator('label').filter(has_text=doc_label).first
                if lbl.count() > 0 and lbl.is_visible(timeout=2000):
                    # Click vào checkbox/radio bên trong
                    inp = lbl.locator('input').first
                    if inp.count() > 0 and not inp.is_checked():
                        lbl.click(force=True, timeout=3000)
                        page.wait_for_timeout(1000)
                    log_cb(f'  ✓ Đã chọn: {doc_label}', 'ok')
                    selected_doc = True
                    break
            except Exception:
                pass
        if not selected_doc:
            log_cb('  ⚠ Không tìm thấy lựa chọn loại phiếu — thử tiếp...', 'warn')

        page.wait_for_timeout(2000)

        # ── 8. Click nút in/xuất phiếu (thường là "In phiếu" hoặc "Xuất phiếu") ──
        state_cb('printing', 'Đang xuất phiếu...')
        export_btn = None
        for btn_text in ['In phiếu', 'In', 'Xuất phiếu', 'Tải phiếu', 'Print']:
            try:
                btn = page.locator(f'button:has-text("{btn_text}")').first
                if btn.count() > 0 and btn.is_visible(timeout=2000):
                    export_btn = btn
        ...
        if export_btn:
            ...
            log_cb(f'  ✓ Đã bấm nút xuất phiếu', 'ok')
            page.wait_for_timeout(3000)
        else:
            log_cb('  ⚠ Không tìm thấy nút xuất phiếu — thử tiếp...', 'warn')
        ...
        def _run():
            ...
            for fp in files:
                try:
                    ...
                    if do_print:
                        _print_file(actual_fp, printer, pdf_settings=pdf_settings, batch_size=batch_size,
